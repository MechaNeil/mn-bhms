...
<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-2">
        ...
        ['icon' => 'fas.money-bill-1', 'value' => 'Php 70, 000', 'title' => 'Monthly Earnings', 'description' => 'This
        month',],
        ['icon' => 'fas.money-bill-alt','value' => 'Php 70, 000', 'title' => 'Total Collectables', 'description' =>
        'Overall',],
        ] as $widgets)


        <x-stat class="transform transition-all duration-300 hover:scale-105" title="{{ $widgets['title'] }}"
            description="{{ $widgets['description'] }}" value="{{ $widgets['value'] }}" icon="{{ $widgets['icon'] }}" />
        @endforeach
    </div>
    <div class="grid lg:grid-cols-6 gap-8 mt-8">
        <div class="col-span-6 lg:col-span-4">
            <x-card title="Chart" shadow separator>
